...
public class DeviceMsgModelWriter extends AExcelWriter {
    public void addHeader(WritableSheet ws) throws WriteException {
        String[] header = {"设备名称","归属机构代码","归属机构名称","设备代号", "采购数量", "产地", "厂家", "设备型号","采购时间","新旧情况","设备价格","使用年限","状态","是否配置GPS","错误信息"};
        if (!"".equals(header)) {
            int i = 0;
            for (String h : header) {
                addCell(ws, 0, i, h);
    ...
    public void write(WritableWorkbook wwb, List ls) throws Exception {
        try {
            ...
            for (DeviceMsgModel m : (List<DeviceMsgModel>) ls) {
                ...
                addCell(ws, i, 2, m.getOrgName(), m.findErrorMsg("orgName"));
                addCell(ws, i, 3, m.getDeviceType(), m.findErrorMsg("deviceType"));
                addCell(ws, i, 4, m.getPurchaseNum(), m.findErrorMsg("purchaseNum"));
                addCell(ws, i, 5, m.getOriginPlace(), m.findErrorMsg("originPlace"));
                addCell(ws, i, 6, m.getManufacturerName(), m.findErrorMsg("manufacturerName"));
                addCell(ws, i, 7, m.getDeviceModel(), m.findErrorMsg("deviceModel"));
                addCell(ws, i, 8, m.getPurchaseTime(), m.findErrorMsg("purchaseTime"));
                addCell(ws, i, 9, m.getIsNew(), m.findErrorMsg("isNew"));
                addCell(ws, i, 10, String.valueOf(m.getDevicePrice()), m.findErrorMsg("devicePrice"));
                addCell(ws, i, 11, String.valueOf(m.getYearLimit()), m.findErrorMsg("yearLimit"));
                addCell(ws, i, 12, m.getStatus(), m.findErrorMsg("status"));
                addCell(ws, i, 13, m.getIsGps(), m.findErrorMsg("isGps"));
                addCell(ws, i, 14, getErrorInfo(m), "");
                i++;
            }
            wwb.write();
            wwb.close();
