...
//        return envelopStr;
    ...
    //-------------------------ICD10与诊断之间关联关系管理 开始-----------------------------------------------------------
    //已关联诊断列表页面
    @RequestMapping("/diagnoseIcd10Relas/initial")
    public String diagnoseIcd10RelaInitial(Model model,String icd10Id){
        model.addAttribute("icd10Id",icd10Id);
        model.addAttribute("contentPage","specialdict/icd10/diagnoseIcd10RelaInfo");
        return "simpleView";
    }
    //新增、修改、查看诊断页面
    @RequestMapping("/diagnoseInfo/initial")
    public String diagnoseIcd10InfoInitial(Model model,String icd10Id,String id,String mode){
        model.addAttribute("icd10Id",icd10Id);
        model.addAttribute("mode",mode);
        Envelop envelop = new Envelop();
        String envelopStr = "";
        try{
            if (!StringUtils.isEmpty(id)) {
                String url = "/dict/icd10/diagnose/" + id;
                envelopStr = HttpClientUtil.doGet(comUrl + url, username, password);
            }
            model.addAttribute("envelop",StringUtils.isEmpty(envelopStr)?objectMapper.writeValueAsString(envelop):envelopStr);
        }catch (Exception ex){
    ...
    @RequestMapping("/diagnose/create")
    @ResponseBody
//  "新增icd10关联的诊断"
    public Object createIcd10DiagnoseRelation(String dataJson,HttpServletRequest request){
        Envelop envelop = new Envelop();
        UserDetailModel userDetailModel = (UserDetailModel)request.getSession().getAttribute(SessionAttributeKeys.CurrentUser);
        try{
            Icd10DiagnoseRelationModel model = objectMapper.readValue(dataJson,Icd10DiagnoseRelationModel.class);
            model.setCreateUser(userDetailModel.getId());
            String url = "/dict/icd10/diagnose";
            Map<String,Object> params = new HashMap<>();
    ...
    @ResponseBody
//  "新增、修改icd10关联的诊断信息"
    public Object updateIcd10DiagnoseRelation(String dataJson,HttpServletRequest request,String mode) throws Exception{
        Envelop envelop = new Envelop();
        envelop.setSuccessFlg(false);
        UserDetailModel userDetailModel = (UserDetailModel)request.getSession().getAttribute(SessionAttributeKeys.CurrentUser);
        String userId = userDetailModel.getId();
        Icd10DiagnoseRelationModel model = objectMapper.readValue(dataJson,Icd10DiagnoseRelationModel.class);
        if(StringUtils.isEmpty(model.getIcd10Id())){
            envelop.setErrorMsg("icd10字典id不能为空！");
            return envelop;
        }
        if(StringUtils.isEmpty(model.getName())){
            envelop.setErrorMsg("诊断名称不能为空！");
            return envelop;
        }
        String url = "/dict/icd10/diagnose";
        try{
            ...
            String urlGet = "/dict/icd10/diagnose/" + model.getId();
            ...
            if(!envelopGet.isSuccessFlg()){
                envelop.setErrorMsg("诊断获取失败！");
                return envelop;
            }
            Icd10DiagnoseRelationModel modelUpdate = getEnvelopModel(envelopGet.getObj(),Icd10DiagnoseRelationModel.class);
            modelUpdate.setUpdateUser(userId);
            modelUpdate.setName(model.getName());
            modelUpdate.setDescription(model.getDescription());
            Map<String,Object> args = new HashMap<>();
            args.put("data_json",objectMapper.writeValueAsString(modelUpdate));
            String envelopStrUpdate = HttpClientUtil.doPut(comUrl+url,args,username,password);
    ...
    @RequestMapping("/diagnoses/delete")
    @ResponseBody
//  "批量、单次删除关联的诊断，多个以逗号隔开"
    public Object deleteIcd10DiagnoseRelations(
            @ApiParam(name = "ids",value = "批量/单次删除关联诊断")
            @RequestParam(value = "ids") String ids){
        Envelop envelop = new Envelop();
        envelop.setSuccessFlg(false);
        if(StringUtils.isEmpty(ids)){
            envelop.setErrorMsg("关联关系id不能为空！");
            return envelop;
        }
        try {
            String url = "/dict/icd10/diagnoses";
            Map<String,Object> params = new HashMap<>();
            params.put("ids",ids);
            String envelopStr = HttpClientUtil.doDelete(comUrl+url,params,username,password);
            return envelopStr;
    ...
    @ResponseBody
//   "获取icd10与诊断关联关联列表，---分页"
    public Object searchIcd10diagnoseRelations(String searchNm,String icd10Id,int rows,int page){
        Envelop envelop = new Envelop();
        if(StringUtils.isEmpty(icd10Id)){
            envelop.setSuccessFlg(false);
            envelop.setErrorMsg("icd10字典id不能为空！");
        ...
        try{
            String filters = "icd10Id="+icd10Id+";";
            if(!StringUtils.isEmpty(searchNm)){
    ...
    @RequestMapping("/diagnoses/no_paging")
    @ResponseBody
//  "查询icd10的诊断关联关系列表，---不分页"
    public Object searchIcd10DiagnoseRelationsWithoutPaging(String searchNm,String icd10Id){
        Envelop envelop = new Envelop();
        if(StringUtils.isEmpty(icd10Id)){
            envelop.setSuccessFlg(false);
            envelop.setErrorMsg("icd10字典id不能为空！");
        ...
        try{
            String url = "/dict/icd10/diagnoses/no_paging";
            Map<String,Object> params = new HashMap<>();
            params.put("filters",filters);
            String envelopStr = HttpClientUtil.doGet(comUrl+url,params,username,password);
            return envelopStr;
    ...
    @ResponseBody
//   "根据关联关系id，获取关联诊断信息"
    public Object getIcd10DiagnoseRelation(String id){
        Envelop envelop = new Envelop();
        String url = "/dict/icd10/diagnose/"+id;
        try{
            String envelopStr = HttpClientUtil.doGet(comUrl+url,username,password);
    ...
    @RequestMapping("/diagnose/isNameExist")
    @ResponseBody
    //同个icd10下的诊断名称不能重复
    public Object isNameExist(String name,String icd10Id){
        Envelop  envelop = new Envelop();
        try{
            String url = "/dict/icd10/diagnoses/no_paging";
            String filters = "icd10Id="+icd10Id+";name="+name+";";
            Map<String,Object> params = new HashMap<>();
            params.put("filters",filters);
            String envelopStr = HttpClientUtil.doGet(comUrl+url,params,username,password);
