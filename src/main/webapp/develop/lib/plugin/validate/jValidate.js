...
(function (Validation, $) {
    ...
    Validation.addAllThese([
        ...
        ['validate-logic-conditions', function (v, elm, args, metadata) {
            if ($(elm).hasClass('mateDate-logic-conditions')) {
                var values = v.replace('，', ',').split(',');
                var logic = ['=', '!=', '<', '<=', '>', '>=', 'IN', 'NOTRANGE', 'RANGE'];
                var bo = true;
                for (var i = 0; i < values.length; i++) {
                    if ($.inArray(values[i], logic) === -1&&values[i] != "") {
                        bo = false;
                        metadata._error = values[i] + "为非法字符请重新收入";
                    }
                }
                return bo;
            } else
