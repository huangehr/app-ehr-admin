...
~(function (w, $) {
    var ET = {
        $main: null,
        html: '<div type="button" class="" id="selView" style="width: 80px;height: 35px;background: #5cb85c;text-align:center;line-height: 35px;cursor:pointer;color: #fff;z-index: 999;position: absolute;top: 0;right: 0">选择视图</div>',
        $sel: '<div id="editTitCon" style="position: absolute;top:0;right: 0;z-index: 999;"><input type="text" id="titVal" style="width: 170px;height: 35px;line-height: 35px;padding: 0 5px;border: 1px solid #5cb85c;border-top:0;right: 80px;display: none;"/><div type="button" class="" data-type="edit" id="editTit" style="width: 80px;height: 35px;display:inline-block;background: #5cb85c;text-align:center;line-height: 35px;cursor:pointer;color: #fff;">编辑</div></div>',
        addViewFun: null,
        ...
        bindEvent: function () {
            ...
            me.$main.on('mouseenter', '.g-title', function (e) {
                ...
                var $that = $(this);
                $that.find('#selView').remove();
                $that.css('border', '1px solid #e8edec');
                e.stopPropagation();
            }).on('click', '.charts #selView', function (e) {
                var $that = $(this);
