...
<script>
	(function ($, win) {
		$(function () {
			...
			var orgCode = backParams.orgCode;
			//角色组已授权资源集合
			var orgRsIds = [];

			// 资源分类；资源明细检索信息，page，pageSize-------------------
			var orgRsPageParams = JSON.parse(sessionStorage.getItem('orgRsPageParams'))
			sessionStorage.removeItem('orgRsPageParams');
			var searchParams = {
				//左侧资源分类树
				categoryId:orgRsPageParams && orgRsPageParams.categoryId || '',
				categorySearchNm:orgRsPageParams && orgRsPageParams.categoryFilter || '',
				//右侧资源列表
				searchNm:orgRsPageParams && orgRsPageParams.resourceFilter || '',
				page:orgRsPageParams && orgRsPageParams.page || 1,
				pageSize:orgRsPageParams && orgRsPageParams.pageSize || 15,
			}
			...
			conditionArea = {
				...
				initOrgMsg: function () {
					this.$orgName.val(backParams.orgName);
			...
			master = {
					...
					this.resourceInfoGrid = $("#div_resource_info_grid").ligerGrid($.LigerGridEx.config({
						...
						columns: [
							...
							{display: '操作', name: 'operator', width: '10%', render: function (row) {
								...
								var html = '<a class="label_a"  href="javascript:void(0)" onclick="javascript:' + Util.format("$.publish('{0}',['{1}','{2}','{3}','{4}'])", "org:resourceManage:list",row.id,row.code,row.name,row.categoryName) + '">维度授权</a>';
								return html;
							}}
						],
				...
				loadResourceIds:function(){
					...
					dataModel.updateRemote("${contextRoot}/organization/orgResourceIds",{
						data:{orgCode:orgCode},
						async:false,
						success: function(data) {
							if(data.successFlg){
								orgRsIds = data.detailModelList;
							}
						}
					});
				},
				...
				savePageParamsToSession:function(){
					...
					sessionStorage.setItem("orgRsPageParams",JSON.stringify(values));
				},
				bindEvents: function () {
					//资源授权
					$('#btn_grant').click(function(){
						$.publish('organization:rs:grant',['']);
					});
					$.subscribe('organization:rs:grant',function(event,ids){
						if(!ids){
							...
							for(var i=0;i<rows.length;i++){
								...
								if(orgRsIds.indexOf(rows[i].id)<0){
									ids += ',' + rows[i].id;
								}
							}
						...
						$.Notice.confirm('确认要授权所选资源？', function (r) {
							if(r){
								var dataModel = $.DataModel.init();
								dataModel.updateRemote('${contextRoot}/organization/resource/OrgGrants',{
									data:{orgCode:orgCode,resourceIds:ids},
									success:function(data){
										if(data.successFlg){
											orgRsIds = [];
											isFirstPage = false;
					...
					$('#btn_grant_cancel').click(function(){
						$.publish("organization:rs:grant:cancel",[''])
					});
					$.subscribe('organization:rs:grant:cancel',function(event,ids){
						if(!ids){
							var rows = master.resourceInfoGrid.getSelectedRows();
							if(rows.length==0){
						...
						$.Notice.confirm('确认要取消授权所选资源？', function (r) {
							if(r){
								var dataModel = $.DataModel.init();
								dataModel.updateRemote('${contextRoot}/organization/resource/cancel',{
									data:{orgCode:orgCode,resourceIds:ids},
									success:function(data){
										if(data.successFlg){
											orgRsIds = [];
					...
					//维度授权页面跳转
					$.subscribe('org:resourceManage:list', function (event,resourceId,code,resourceName,categoryName) {
						...
						var data = {
							...
							'backParams':backParams,//资源页面顶org信息
						}
						$("#contentPage").empty();
						$("#contentPage").load('${contextRoot}/organization/resourceManage/initial?orgCode='+orgCode+'&resourceId='+resourceId,{dataModel:JSON.stringify(data)});
					});
				},
			};
