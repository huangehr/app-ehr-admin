...
//                    formData:{msg:"upload"},
            ...
            initAddress: function (target, value) {
                target.addressDropdown({
                    tabsData: [
                        {
                            name: '省份',
                            code: 'id',
...
//                var idCardNo = self.$form.Fields.idCardNo.getValue();
                ...
                patientInfo.$updateBtn.click(function () {
                    if ($(".tab-list li.cur").html() == "角色授权") {//保存角色授权值
                        ...
                        var dataModel = $.DataModel.init();
                        dataModel.updateRemote('${contextRoot}/patient/appUserRolesSave', {
                            data: {userId: userId, jsonData: JSON.stringify(saveData)},
                            success: function (data) {
                                wait.close();
                                if (data.successFlg) {
                                    $.Notice.success('保存成功！');
...
//                        updatePatient(JSON.stringify(values));
        ...
        cardFormInit = {
            ...
            searchCard: function (searchNm, cardType) {
                cardInfoGrid.setOptions({
                    parms: {searchNm: searchNm, idCardNo: idCardNo, cardType: cardType},
                    newPage: 1
                });
