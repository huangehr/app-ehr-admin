			...
			masters = {
				...
				init:function(){
					infoGrid = $('#div-special-dict-grid').ligerGrid($.LigerGridEx.config({
						url: '${contextRoot}/specialdict/drug/drugs',
						parms: {
							searchNm: ''
						},
						columns: [
							...
							{display: '商品名', name: 'tradeName', width: '20%',align:'left'},
							...
							{display: '规格', name: 'specifications', width: '10%', align: 'center'},
							{
								...
								html ='<a class="grid_edit" name="delete_click" style="" title="编辑" onclick="javascript:' + Util.format("$.publish('{0}',['{1}','{2}'])", "drug:info:open", row.id,'modify') + '"></a>'
										+'<a class="grid_delete" name="delete_click" style="" title="删除"' +
										' onclick="javascript:' + Util.format("$.publish('{0}',['{1}'])", "drug:info:delete", row.id) + '"></a>';
								return html;
							}},
						],
						//enabledEdit: true,
				...
				bindEvents:function(){
					...
					$.subscribe('drug:info:open',function(event,id,mode){
						...
						self.infoDialog = $.ligerDialog.open({
							...
							width:500,
							title:title,
							url:'${contextRoot}/specialdict/drug/dialog/drugInfo',
							urlParms:{
								id:id,
								mode:mode,
							},
					...
					$('#btn_deletes').click(function(){
						$.publish('drug:info:delete',['']);
					});
					$.subscribe('drug:info:delete',function(event,ids){
						if(!ids){
