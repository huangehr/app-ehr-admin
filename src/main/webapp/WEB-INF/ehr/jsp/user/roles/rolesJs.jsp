					...
					this.rolesGrid = $("#div_roles_grid").ligerGrid($.LigerGridEx.config({
						...
						columns: [
							...
							{display: '描述', name: 'description', width: '30%', isAllowHide: false, align: 'center'},
							{
								...
								var html = '<sec:authorize url=""><a class="label_a" href="javascript:void(0)" onclick=javascript:' + Util.format("$.publish('{0}',['{1}','{2}'])", "roles:config:open", jsonStr,"limits") + '>权限配置</a></sec:authorize>';
								html += '<sec:authorize url=""><a class="label_a" style="margin-left:15px" href="javascript:void(0)" onclick=javascript:' + Util.format("$.publish('{0}',['{1}','{2}'])", "roles:config:open", jsonStr,"users") + '>人员配置</a></sec:authorize>';
								html += '<sec:authorize url=""><a class="grid_edit" title="编辑" href="javascript:void(0)" onclick="javascript:' + Util.format("$.publish('{0}',['{1}','{2}'])", "roles:infoDialog:open", row.id, 'modify') + '"></a></sec:authorize>';
								html+= '<sec:authorize url=""><a class="grid_delete" title="删除" href="javascript:void(0)" onclick="javascript:' + Util.format("$.publish('{0}',['{1}','{2}'])", "roles:info:delete", row.id, 'delete') + '"></a></sec:authorize>';
								return html;
							}}
						],
						//usePager:false,
