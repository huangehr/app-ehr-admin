...
//                $("#contentPage").empty().load(url,data);
			...
			var searchParams = {
				//左侧资源分类树
				categoryId:rolesRsPageParams && rolesRsPageParams.categoryId || '',
				categorySearchNm:rolesRsPageParams && rolesRsPageParams.categoryFilter || '',
				//右侧资源列表
				searchNm:rolesRsPageParams && rolesRsPageParams.resourceFilter || '',
				page:rolesRsPageParams && rolesRsPageParams.page || 1,
				pageSize:rolesRsPageParams && rolesRsPageParams.pageSize || 15,
			}
			...
			/* *************************** 模块初始化 ***************************** */
			conditionArea = {
				$rolesName :$('#msg_rolesName'),
				$catalogName :$('#msg_catalogName'),
				$orgName :$('#msg_orgName'),
				init : function () {
					this.initRolesMsg();
				},
				initRolesMsg: function () {
					this.$rolesName.val(backParams.rolesName);
			...
			master = {
					...
					this.resourceInfoGrid = $("#div_resource_info_grid").ligerGrid($.LigerGridEx.config({
						...
						columns: [
							...
							{display: '操作', name: 'operator', width: '10%', render: function (row) {
								...
								var html = '<a class="label_a"  href="javascript:void(0)" onclick="javascript:' + Util.format("$.publish('{0}',['{1}','{2}','{3}','{4}'])", "roles:resourceManage:list",row.id,row.code,row.name,row.categoryName) + '">维度授权</a>';
								return html;
							}}
						],
				...
				loadResourceIds:function(){
					...
					dataModel.updateRemote("${contextRoot}/userRoles/rolesResourceIds",{
						data:{rolesId:rolesId},
						async:false,
						success: function(data) {
							if(data.successFlg){
								rolesRsIds = data.detailModelList;
							}
						}
					});
				},
				...
				savePageParamsToSession:function(){
					...
					sessionStorage.setItem("rolesRsPageParams",JSON.stringify(values));
				},
				bindEvents: function () {
					//资源授权
					$('#btn_grant').click(function(){
						$.publish('roles:rs:grant',['']);
					});
					$.subscribe('roles:rs:grant',function(event,ids){
						if(!ids){
							...
							for(var i=0;i<rows.length;i++){
								...
								if(rolesRsIds.indexOf(rows[i].id)<0){
									ids += ',' + rows[i].id;
								}
							}
						...
						$.Notice.confirm('确认要授权所选视图？', function (r) {
							if(r){
								...
								dataModel.updateRemote('${contextRoot}/userRoles/resource/rolesGrant',{
									data:{rolesId:rolesId,resourceIds:ids},
									success:function(data){
										if(data.successFlg){
											rolesRsIds = [];
											isFirstPage = false;
					...
					$('#btn_grant_cancel').click(function(){
						$.publish("roles:rs:grant:cancel",[''])
					});
					$.subscribe('roles:rs:grant:cancel',function(event,ids){
						if(!ids){
							var rows = master.resourceInfoGrid.getSelectedRows();
							if(rows.length==0){
                    ...
                    $.subscribe('app:rs:grant:all',function(event,ids){
                        ...
                        parent._LIGERDIALOG.confirm('确认要授权所有视图？', function (r) {
                            if(r){
                                ...
                                dataModel.updateRemote('${contextRoot}/appRole/rolesGrantResourcesByCategoryId',{
                                    data:{rolesId:rolesId,appId:appId,categoryIds:categoryId,resourceIds:ids},
                                    success:function(data){
                    ...
                    $.subscribe('app:rs:grant:cancel:all',function(event,ids){
                        ...
                        parent._LIGERDIALOG.confirm('确认要取消所有视图的授权？', function (r) {
                            if(r){
                                var dataModel = $.DataModel.init();
                                dataModel.updateRemote('${contextRoot}/appRole/deleteRolesGrantResourcesByCategoryId',{
                                    data:{rolesId:rolesId,appId:appId,categoryIds:categoryId,resourceIds:ids},
                                    success:function(data){
                                        if(data.successFlg){
                                            appRsIds = [];
